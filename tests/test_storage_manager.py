...
def test_add_and_get_cycle(storage):
    cycle_id = storage.add_cycle(start_date=date(
        2026, 1, 1), end_date=date(2026, 1, 5))
...
def test_daily_log_overwrite(storage):
    storage.add_daily_log(log_date=date(2026, 1, 2),
                          flow_intensity="Light", symptoms="", mood="Happy")
    storage.add_daily_log(log_date=date(2026, 1, 2),
                          flow_intensity="Heavy", symptoms="Cramps", mood="Sad")
...
def test_database_encryption():
    ...
    try:
        ...
        raw_cursor.execute(
            "SELECT flow_intensity, symptoms, mood, notes FROM daily_logs WHERE date=?", (log_date.isoformat(),))
