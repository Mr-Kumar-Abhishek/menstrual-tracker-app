...
| **NFR3: Portability** | ✅ Compliant | Built with Python 3 and Kivy/KivyMD, fulfilling cross-platform (Windows, Linux, Android, iOS) needs with a single codebase. Configured for **Android Target API Level 36**, 32 KB memory page size alignment (`-Wl,-z,max-page-size=32768`), and automated version code calculation (`> 400`) in GitHub Actions CI/CD (`build-and-release.yml`). |
...
> NFR3 (Portability & Modern Android Support) is compliant with target Android **API Level 36** (`android.api = 36`), 32 KB memory page size alignment (`LDFLAGS="-Wl,-z,max-page-size=32768"`), and Play Store versioning standards (`android.numeric_version` set > 400).
