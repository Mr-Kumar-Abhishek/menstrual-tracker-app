...
- **Build & Packaging Tools:** PyInstaller (Windows `.exe`, Linux binaries), Buildozer (Android `.aab` targeting **API Level 36** with 16 KB memory page size alignment `-Wl,-z,max-page-size=16384` and dynamic version code > 300), Toolchain (`kivy-ios` for iOS `.xcarchive`).
